...
func move_horizontally(delta:float, direction:Vector2):
	if direction:
		horizontal_speed += AIR_ACCELERATION * delta
	else:
		horizontal_speed -= AIR_DECCELERATION * delta
	horizontal_speed = clamp(horizontal_speed, 0, max_horizontal_speed)
	var target_velocity :Vector2= horizontal_speed * direction.normalized()
	var steering_velocity :Vector2= (target_velocity - horizontal_velocity).normalized() * AIR_STEERING_POWER
